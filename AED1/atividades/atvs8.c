...
void exibirUmaPilha(PILHATRIPLA *p, int pilha)
{
    ...
    if (pilha == 1)
        ...
        for (i = p->topo2; i >= p->base2; i--)
            printf("%i ", p->A[i].chave);
    else
        for (i = p->topo3; i < MAX; i++)
            printf("%i ", p->A[i].chave);
    printf("\"\n");
} /* exibirPilha */
...
{
    if (p->base2 == p->topo1 + 1)
        return false;
    int x;
    for (x = p->base2; x <= p->topo2; x++)
        p->A[x - 1] = p->A[x];
    p->base2--;
    p->topo2--;
    return true;
...
bool inserirElementoPilha(PILHATRIPLA *p, REGISTRO reg, int pilha)
{
    if (pilha < 1 || pilha > 3)
        return false;
    if (tamanhoPilhaTripla(p) == MAX)
        return false;
    if (pilha == 1)
    {
        if (p->topo1 == p->base2 - 1)
            deslocarADireita(p);
        p->topo1 = p->topo1 + 1;
        p->A[p->topo1] = reg;
    }
...
int main()
{
    ...
    inicializarPilhaTripla(&p);
    //Falha
    printf("%i \n", excluirElementoPilha(&p, &r, 1));
    printf("%i \n", excluirElementoPilha(&p, &r, 2));
    printf("%i \n\n", excluirElementoPilha(&p, &r, 3));

    r.chave = 2;
    inserirElementoPilha(&p, r, 1);
    inserirElementoPilha(&p, r, 2);
    inserirElementoPilha(&p, r, 3);
    // Funciona
    printf("%i \n", excluirElementoPilha(&p, &r, 1));
    printf("%i \n", excluirElementoPilha(&p, &r, 2));
    printf("%i \n", excluirElementoPilha(&p, &r, 3));
