...
int posMenor(LISTA * l, int inicio){
    int i, menor; 
    menor = -1; 
    if((inicio >= 0) && (inicio < l->nroElem)){
        menor = inicio;
        for(i = inicio+1; i < l->nroElem; i++){
            if(l->A[i].chave < l->A[menor].chave)   
                menor = i; 
        }
    }
    return menor;
...
void ordernarSelection (LISTA * l){
    int i, menor, aux;

    for(i = 0; i < l->nroElem; i++){
        menor = posMenor(l, i);
        aux = l->A[i].chave;
        l->A[i].chave = l->A[menor].chave;
        l->A[menor].chave = aux; 
    }
}
