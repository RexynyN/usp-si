...
int arr_to_matriz(float* array, float** X, float* Y, int len) {
    int stop = len - SLIDE;
    // Aloca memória para X (matriz de entrada)
    for(int idx = 0; idx < stop; idx++) {
        int slider = idx + SLIDE;
        // Copia os elementos para a matriz X
        for (int j = 0; j < SLIDE; j++) {
            X[idx][j] = array[idx + j];
        }

        int prev_idx = slider + (PREV - 1); // cálculo do índice anterior
        if (Y != NULL && prev_idx < len)
            Y[idx] = array[prev_idx];  // Valor de Y (0 ou valor do índice)
    }
...
int* get_min_idxs(float* array, int len) {
    ...
    int shift = 0; // An index shift to reflect the original array's indexes
    for (int k = 0; k < KNN; k++) {
        float min = array[0];
        int min_idx = 0;
        // Do a simple loop to find the smallest 
        for(int i = 0; i < len; i++) {
            if (array[i] < min) {
                min = array[i];
                min_idx = i;
        ...
        // Remove the min we just got, making the array smaller
...
float * temporal_knn(float **xtrain, float** xtest, float* ytrain) {
    ...
    for (int i = 0; i < TEST_LEN; i++) {
        ...
        int *sort_idxs = get_min_idxs(dists, TRAIN_LEN);
        float regr = 0.0;
        for(int x = 0; x < KNN; x++) {
